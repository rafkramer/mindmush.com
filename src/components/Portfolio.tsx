  ...
  return (
    <section id="portfolio" className="snap-section px-4 sm:px-6">
      <div className="max-w-5xl mx-auto w-full">
        ...
        <div className="flex lg:hidden gap-3 overflow-x-auto pb-4 -mx-4 px-4 scrollbar-hide mb-4">
          {launches.map((launch) => (
            <button
              key={launch.id}
        ...
        {/* Desktop Layout - Card + Phone side by side */}
        <div className="hidden lg:grid lg:grid-cols-[1fr_180px] gap-12 items-center">
          {/* Detail Card */}
          <motion.div
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`text-[10px] font-medium px-2.5 py-1 rounded-full ${status.bg} ${status.text}`}>
                    {activeLaunch.metric}
                  </span>
                </div>
          ...
          >
            <div
              className="relative bg-[#1a1a1c] rounded-[32px] p-2 shadow-2xl"
              style={{ width: '160px' }}
            >
              {/* Screen */}
              <div className="bg-black rounded-[26px] overflow-hidden" style={{ height: '320px' }}>
                {/* Notch */}
                <div className="flex justify-center pt-2">
                  <div className="w-16 h-5 bg-black rounded-full" />
                </div>

                {/* App Grid - 3x2 for the 6 apps */}
                <div className="px-3 pt-4">
                  <div className="grid grid-cols-3 gap-2">
                    {launches.map((launch) => {
                      const isActive = activeId === launch.id;
                      return (
                        <button
                          key={launch.id}
                          onClick={() => setActiveId(launch.id)}
                        ...
                        >
                          <div
                            className={`w-10 h-10 rounded-xl overflow-hidden transition-all duration-200 ${
                              isActive
                                ? 'ring-2 ring-cyan-400 ring-offset-1 ring-offset-black scale-105'
                                : 'group-hover:scale-105'
                            }`}
                          >
                            <img src={launch.icon} alt={launch.name} className="w-full h-full object-cover" />
                          </div>
                          <span className={`text-[8px] truncate w-10 text-center transition-colors ${
                ...
                {/* Home Indicator */}
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 w-20 h-1 bg-white/60 rounded-full" />
              </div>
            </div>
          </motion.div>
        </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold text-white">{activeLaunch.name}</h3>
              </div>
              <p className="text-sm text-white/50 leading-relaxed">{activeLaunch.description}</p>
              {activeLaunch.link && (
                <div className="mt-4 pt-4 border-t border-white/[0.06]">
                  <a href={activeLaunch.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-xs text-white/40 hover:text-cyan-400 transition-colors">
                    View on {activeLaunch.linkLabel}
