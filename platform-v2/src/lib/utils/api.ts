...
export async function testApplovinApi(apiKey: string): Promise<{ success: boolean; error?: string }> {
  try {
...
export async function fetchApplovinRevenue(
  bundleId: string,
  startDate: string,
  endDate: string
): Promise<number | null> {
    ...
    if (response.ok) {
      ...
      if (data.results && data.results.length > 0) {
        return data.results.reduce(
          (sum: number, row: { estimated_revenue?: number }) =>
            sum + (parseFloat(String(row.estimated_revenue)) || 0),
          0
        );
      }
    }
...
export async function fetchSuperwallRevenue(
  apiKey: string,
  startDate: string,
  endDate: string
): Promise<number | null> {
...
export async function syncAllVentures(): Promise<SyncResult> {
...
export async function syncVenture(ventureId: string): Promise<{ success: boolean; revenue?: number; error?: string }> {
